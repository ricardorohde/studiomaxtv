...
class AdminEvento {
    ...
    /**
     * <b>Cadastrar Eventos:</b> Envelopa os dados em um array atribuitivo e execute este método
    ...
    public function ExeCreate(array $Data) {
        ...
        if ($this->Data['foto']):
            $upload = new Upload;
            $upload->Image($this->Data['foto'], $this->Data['evento'], 640, 'eventos');
        endif;
    ...
    /**
     * <b>Atualizar Eventos:</b> Envelope os dados em uma array atribuitivo e informe o id de uma
     * noticia para atualiza-la no sistema!
     * @param INT $EventoId = Id do Evento
     * @param ARRAY $Data = Atribuitivo
     */
    public function ExeUpdate($EventoId, array $Data) {
        $this->Id = (int) $EventoId;
        $this->Data = $Data;
        $this->checkData();
        $this->setData();
    ...
    /**
     * <b>Remover Evento:</b> Informe o ID da noticia que deseja remover.
     * @param INT $EventoId = Id do Evento
     */
    public function ExeDelete($EventoId) {
        $this->Id = (int) $EventoId;
        ...
        if (!$readVideo->getResult()):
            $this->Error = ['Oppsss, você tentou remover um evento que não existe no sistema!', WS_ERROR];
            $this->Result = false;
        else:
            $this->fotoDelete($this->Id);
            $this->Delete();
    ...
    private function Create() {
        ...
        if ($Create->getResult()):
            $this->Error = ["O evento <b>{$this->Data['evento']}</b> foi cadastrado com sucesso no sistema!", WS_ACCEPT];
    ...
    private function Update() {
        ...
        if ($Update->getResult()):
            $this->Error = ["O evento <b>{$this->Data['nome']}</b> foi atualizado com sucesso!", WS_ACCEPT];
            $this->Result = true;
        endif;
    }
